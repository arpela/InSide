...
import uy.com.s4b.inside.core.common.TypeDevice;
...
import uy.com.s4b.inside.core.entity.Model;
...
public class UtilCreatePage {
			...
			for (Site site : network.getColSite()) {
				...
				for (Zone zone : site.getColZone()) {
					...
					for (Device device : zone.getColDevice()) {
						String ico = getIcono(device.getModel());
						retorno.append("<li style=\"background-image: url('"+ nameAPP +"/resources/img/"+ ico +"')\">"); 
	 ...
	 * @return
	 */
	private String getIcono(Model model) {
		if (model.getName() == TypeDevice.Firewall){			
			return "firewalls.jpg";
		} else if (model.getName() == TypeDevice.Router){
			return "router.jpg";
		} else {
			return "switch.jpg";
		}
	...
	private void putConfiguration(StringBuffer p, Device device, String nameAPP) {
		...
		if (listVersino != null){
			for (Version version : listVersino) {
				...
				p.append("<li style=\"background-image: url('" + nameAPP + "/resources/img/" + ico + "')\">");
	...
	public void createHomeListaEvento(StringBuffer retorno,
		...
		if ((listaEventos != null) && (!listaEventos.isEmpty())){
			Collections.sort(listaEventos,  new Comparator<EventInSide> (){
				@Override
				public int compare(EventInSide e1, EventInSide e2) {
